...
use http::HeaderMap;
use jsonwebtoken::{decode, decode_header, jwk::JwkSet, Algorithm, DecodingKey, TokenData, dangerous};
    ...
    C: DeserializeOwned + Clone + Send,
{
    ...
    pub async fn check_auth(&self, token: &str) -> Result<TokenData<C>, AuthError> {
        ...
        let token_data = if self.validation.validate_signature {
            decode::<C>(token, &val_key.key, jwt_validation)?
        } else {
            dangerous::insecure_decode::<C>(token)?
        };
