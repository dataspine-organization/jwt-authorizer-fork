...
mod tests {
    ...
    async fn extract_from_public_optional() {
        // In axum 0.8, Option extractors don't work with our custom rejection type
        // So we manually extract from extensions instead
        let app = Router::new().route(
            "/public",
            get(|parts: axum::extract::Request| async move {
                let user = parts.extensions().get::<jsonwebtoken::TokenData<User>>();
                format!("option: {}", user.is_none())
            }),
        );
        let response = app
            .oneshot(Request::builder().uri("/public").body(Body::empty()).unwrap())
